...
public:
  ...
  struct vertex_type
  {
    ...
    /** \brief Constructs a vertex_type object with local vid
       ...
       */
    vertex_type(distributed_graph &graph_ref, lvid_type lvid) : graph_ref(graph_ref), lvid(lvid) {}
    /// \endcond
  ...
  private:
    ...
    edge_type(distributed_graph &graph_ref,
              typename local_graph_type::edge_type edge) : graph_ref(graph_ref), edge(edge) {}
  ...
  distributed_graph(distributed_control &dc,
                    const graphlab_options &opts = graphlab_options()) : rpc(dc, this), finalized(false), vid2lvid(),
...
#pragma omp parallel for
  ...
  {
    // read the vertices
    arc >> nverts >> nedges >> local_own_nverts >> nreplicas >> vid2lvid >> lvid2record >> local_graph;
    finalized = true;
                       ...
                       size_t files_per_machine = 4)
  {
    ...
    for (size_t i = 0; i < files_per_machine; ++i)
    {
      ...
      graph_files[i] = prefix + "." + tostr(1 + i + rpc.procid() * files_per_machine) + "_of_" + tostr(rpc.numprocs() * files_per_machine);
      if (gzip)
        graph_files[i] += ".gz";
    }
    ...
    if (save_vertex)
      parallel_for_vertices(vertex_callbacks);
    if (save_edge)
      parallel_for_edges(edge_callbacks);
                    ...
                    size_t files_per_machine = 4)
  {
    ...
    for (size_t i = 0; i < files_per_machine; ++i)
    {
      graph_files[i] = prefix + "_" + tostr(1 + i + rpc.procid() * files_per_machine) + "_of_" + tostr(rpc.numprocs() * files_per_machine);
      if (gzip)
        graph_files[i] += ".gz";
    }
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < graph_files.size(); ++i)
    {
      if ((parallel_ingress && (i % rpc.numprocs() == rpc.procid())) || (!parallel_ingress && (rpc.procid() == 0)))
      {
...
#pragma omp parallel for
  ...
  void load_format(const std::string &path, const std::string &format)
  {
    ...
    {
      line_parser = builtin_parsers::custom_parser<distributed_graph>;
      load(path, line_parser);
    }
    else
    {
 ...
 ****************************************************************************/
  ...
  struct vertex_record
  {
    ...
    vertex_record() : owner(-1), gvid(-1), num_in_edges(0), num_out_edges(0) {}
    vertex_record(const vertex_id_type &vid) : owner(-1), gvid(vid), num_in_edges(0), num_out_edges(0) {}
    ...
    {
      clear();
      arc >> owner >> gvid >> num_in_edges >> num_out_edges >> _mirrors;
    }
...
#pragma omp parallel for
  ...
  struct local_vertex_type
  {
    ...
    local_vertex_type(distributed_graph &graph_ref, lvid_type lvid) : graph_ref(graph_ref), lvid(lvid) {}
  ...
  public:
    local_edge_type(distributed_graph &graph_ref,
                    typename local_graph_type::edge_type e) : graph_ref(graph_ref), e(e) {}
  ...
  struct make_local_edge_type_functor
  {
    ...
    distributed_graph &graph_ref;
    make_local_edge_type_functor(distributed_graph &graph_ref) : graph_ref(graph_ref) {}
  ...
  struct local_edge_list_type
  {
    ...
    local_edge_list_type(distributed_graph &graph_ref,
                         typename local_graph_type::edge_list_type elist) : me_functor(graph_ref), elist(elist) {}
    ...
    /** \brief Returns an iterator to the beginning of the list.
       ...
       *
      */
    iterator begin() const { return boost::make_transform_iterator(elist.begin(), me_functor); }
    ...
    /** \brief Returns an iterator to the end of the list.
       ...
       *
      */
    iterator end() const { return boost::make_transform_iterator(elist.end(), me_functor); }
...
private:
                          ...
                          size_t bufsize = 50000, bool usehash = false, bool userecent = false)
  {
    ...
    {
      if (rpc.procid() == 0)
        logstream(LOG_EMPH) << "Use hdrf oblivious ingress, usehash: " << usehash
                            << ", userecent: " << userecent << std::endl;
